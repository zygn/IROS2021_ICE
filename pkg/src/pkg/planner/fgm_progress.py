...
class FGM_p:
    def __init__(self, params=None):
        ...
        if params == None:
            self.params = {
                ...
                'waypoint_path': 'pkg/SOCHI_0925.csv'  # for Python main ENVIRONMENT
            }
        else:
            self.params = params
        #%
    ...
    def main_drive(self, max_gap):
        ...
        if self.ovt:
            if (self.wp_index_current >= 400 and self.wp_index_current <= 750) or (self.wp_index_current >= 3100 and self.wp_index_current <= 3600):
                ...
                # print(f"current : {self.current_speed}, speed: {speed}, idx: {self.wp_index_current}")
            else:
                ...
                speed = self.speed_control.obs_following(self.current_speed, steering_angle, self.len_obs[0][5]*0.1)
                print(f"current : {self.current_speed}, speed: {speed}, idx: {self.wp_index_current}")
    ...
    def _process_lidar(self, scan_data, odom_data):
        ...
        if self.ovt==False:
            ...
            # print(self.ovt, self.best_point,self.desired_wp_rt)
        else:
            # print(self.ovt)
            self.ovt_flag = True
            # self.desired_gap = self.find_ovt_gap()
            # self.best_point = (self.desired_gap[0]+self.desired_gap[1])/2
            # print(self.ovt, self.best_point, self.gaps , self.desired_gap, self.past_point)
        self.desired_gap = self.find_best_gap(self.desired_wp_rt)
        #print(self.ovt)
        self.best_point = self.find_best_point(self.desired_gap)
    ...
    def main_drive(self, max_gap):
        ...
        if self.ovt:
            if (self.wp_index_current >= 400 and self.wp_index_current <= 750) or (self.wp_index_current >= 3100 and self.wp_index_current <= 3600):
                ...
                speed = self.speed_control.obs_following(self.current_speed, steering_angle, self.len_obs[0][5]*0.1)
                # ^
                # print(self.scan_data == self.callbacked_data)
                # print(f"current : {self.current_speed}, speed: {speed}, idx: {self.wp_index_current}")
                # speed = self.speed_control.routine(self.scan_filtered, self.current_speed, steering_angle, self.wp_index_current, 0)
    ...
    def _process_lidar(self, scan_data, odom_data):
        ...
        if self.ovt:
            if (self.wp_index_current >= 0 and self.wp_index_current <= 750) or (self.wp_index_current >= 3100 and self.wp_index_current <= 3600):
                ...
                self.best_point = (self.desired_gap[0]+self.desired_gap[1])/2
                if len(self.gaps) > 1:
                    print(f"Gaps: {self.gaps} | Desired Gap: {self.desired_gap}")
                    for i in range(0, len(self.gaps)):
                        if self.gaps[i] == self.desired_gap
                        print(f"{i} Gap Data Mean: {np.mean(self.scan_filtered[self.gaps[i][0]:self.gaps[i][1]])}")
                ...
                self.best_point = self.find_best_point(self.desired_gap)
                # ^^
                # print(self.gaps, self.desired_gap, self.best_point, self.past_point)
        else: 
            self.desired_gap = self.find_best_gap(self.desired_wp_rt)
            self.best_point = self.find_best_point(self.desired_gap)
            #print(self.ovt, self.best_point,self.desired_wp_rt)
